Com['Slider'] = function(o){
	...
	var renderSlide = function(i, item){
        ...
        if(style = cm.getSupportedStyle('transition-duration')){
            slide['node'].style[style] = [config['time'], 'ms'].join('');
        }
	...
	var set = function(item){
        ...
        if(is('IE') && isVersion() < 10){
            slides[item]['anim'].go({'style' : {'opacity' : 1}, 'duration' : config['time']});
        }else{
            hack = slides[item]['node'].clientHeight;
            slides[item]['node'].style.opacity = 1;
        }
	...
	var fadeOut = function(slide){
		...
		if(slides[current]){
			...
			if(config['fadePrevious']){
                if(is('IE') && isVersion() < 10){
                    slides[slide]['anim'].go({'style' : {'opacity' : 0}, 'duration' : config['time']});
                }else{
                    hack = slides[slide]['node'].clientHeight;
